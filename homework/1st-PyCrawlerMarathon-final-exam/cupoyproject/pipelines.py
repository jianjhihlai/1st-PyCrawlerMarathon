...
class CupoyprojectPipeline(object):
    def open_spider(self, spider):
        ...
        self.runtime_file_path = self.runtime_file_path.format(spider.group_id)
    ...
    def close_spider(self, spider):
        ...
        # 以爬蟲的 group_id + 日期當作存檔檔名
        if spider.name == 'PTTCrawler' and spider.group_id:
            self.store_file_path = self.dir_path / '{group_id}-{datetime}.json'.format(
                    group_id=spider.group_id, datetime=datetime.now().strftime('%Y%m%dT%H%M%S'))
